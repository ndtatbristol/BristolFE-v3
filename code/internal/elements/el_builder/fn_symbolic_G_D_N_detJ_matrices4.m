function [G, D, N, detJ, W, loc_nd, loc_df, start_lines, end_lines] = fn_symbolic_G_D_N_detJ_matrices4(nds_in_nat_coords, gauss_pts, gauss_weights, sf_powers, solid_or_fluid, varargin)
...
N = fn_symbolic_shape_functions(nds_in_nat_coords, sf_powers, Q);

keyboard
dNdQ = sym('dNdQ_%d_%d', [no_dims_of_el, no_dims_of_el], 'real');
for i = 1:no_dims_of_el
    for j = 1:no_dims_of_el
        dNdQ(i, j) = diff(N(i), Q(j));
    end
end

%Get Jacobian, inverse Jacobian and detJ
Ntmp = fn_symbolic_shape_function_matrix(N, no_dims_of_el);
J = jacobian(Ntmp * reshape(nds', [], 1), Q);
...
[detJ_gen, invJ] = fn_symbolic_inv_jacobian(N, nds, Q, no_dims_of_el);
