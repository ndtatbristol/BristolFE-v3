function [B, N, detJ, W, loc_nd, loc_df, start_lines, end_lines] = fn_symbolic_B_N_detJ_matrices4(nds_in_nat_coords, gauss_pts, gauss_weights, sf_powers, solid_or_fluid, varargin)
%This version returns B in factorised form
if numel(varargin) < 1
...
fprintf('Generating shape derivatives\n')
B3_gen = sym('N_diff_%d_%d', [no_spatial_dims, numel(shape_functions)], 'real');

for i = 1:no_spatial_dims
    for j = 1:numel(shape_functions)
        if i <= numel(Q)
            B3_gen(i, j) = diff(shape_functions(j), Q(i));
        else
            B3_gen(i, j) = 0;
        end
    end
...
[detJ_gen, B2_gen] = fn_symbolic_inv_jacobian(shape_functions, nds, Q, no_dims_of_el);
B2_gen = [[B2_gen, zeros(no_dims_of_el, no_spatial_dims - no_dims_of_el)]; zeros(no_spatial_dims - no_dims_of_el, no_spatial_dims)];
